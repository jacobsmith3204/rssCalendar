...
const REDIRECT_URL = process.env.GOOGLE_OAUTH_CALLBACK || "https://localhost:8000/oauth/oauthcallback"; // allows an optional .env callback
...
export class OauthHandler extends BaseHandler {
    ...
    HandleGet(client: TcpClient) {
        ...
        switch (pathname) {
            case 'startoauth':
                client.SendResponse(200, { oauthurl: generateLoginURL() });
                break;
            case 'oauthcallback':
...
export async function LoadAllClientLoginsFromFile() {
    ...
    if (fs.existsSync(LOGINS_PATH)) {
        ...
        // creates a blank file. then finishes by reading the blank file
